...
from .decorators import user_is_patient, user_is_vet, user_is_viewer
from django.views.generic import TemplateView, UpdateView, CreateView, ListView, DetailView, DeleteView
from django.views.generic.edit import DeleteView, UpdateView
from accounts.forms import PatientProfileUpdateForm, VetProfileUpdateForm, ViewProfileUpdateForm
...
class EditViewProfileView(UpdateView):
    model = User
    form_class = ViewProfileUpdateForm
    context_object_name = 'patient'
    template_name = 'accounts/view_user/edit-profile.html'
    success_url = reverse_lazy('accounts:view-profile-update')
    ...
    def get_object(self, queryset=None):
        ...
        print(obj)
        if obj is None:
            raise Http404("Viewer doesn't exists")
        return obj
...
# # experiment update+
# class TakeAppointmentView(UpdateView):
#     template_name = 'appointment/take_appointment.html'
#     form_class = TakeAppointmentForm
#     extra_context = {
#         'title': 'Take Appointment'
...
class EditVetProfileView(UpdateView):
    model = User
    form_class = VetProfileUpdateForm
    context_object_name = 'vet'
    template_name = 'accounts/vet/edit-profile.html'
    success_url = reverse_lazy('accounts:vet-profile-update')

    @method_decorator(login_required(login_url=reverse_lazy('accounts:login')))
    @method_decorator(user_is_vet)
    def dispatch(self, request, *args, **kwargs):
        return super().dispatch(self.request, *args, **kwargs)
...
class AppointmentCreateView(CreateView):
    ...
    success_url = reverse_lazy('appointment:vet-appointment')
...
class viewlist(ListView):
    model = TakeAppointment
    context_object_name = 'patients'
    template_name = "appointment/views.html"

class viewlistuser(ListView):
    model = TakeAppointment
    context_object_name = 'patients'
    template_name = "appointment/view-list.html"
...
class PatientUpdateView(UpdateView):
    ...
    success_url = reverse_lazy('appointment:patient-list')
#updating 1 more  patientview grinding 
...
class AppointmentDeleteView(DeleteView):
    """
       For Delete any Appointment created vet
    """
    model = Appointment
    success_url = reverse_lazy('appointment:vet-appointment')
